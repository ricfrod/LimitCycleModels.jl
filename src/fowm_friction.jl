...
function (self::FowmFriction)(u::AbstractVector{<:Real})
    (
        ; ρ_l, T, M, θ, ρ_mres, H_pdg, H_vgl,
        m_lstill, A, Vᵣ, Vₜ, R, g
    ) = self
    _, m_gr, m_lr, _, m_gt, m_lt, ΔPₜ, ΔPᵣ = u
...
function (self::FowmFriction)(u::AbstractVector{<:AbstractVector{T}})::Matrix{T} where {T <: Real}
